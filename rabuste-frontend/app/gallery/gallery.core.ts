...
const ROOT_ID = "drag-gallery-root";

function getRoot(): HTMLElement {
  const el = document.getElementById(ROOT_ID);
  if (!el) throw new Error("DragGallery root not found");
  return el;
}
...
class PreloaderManager {
  constructor() {
    ...
    this.startTime = null;
    // no artificial delay — finish immediately (0). Set a small value (e.g. 300) if you want a short animation.
    this.duration = 0;
...
class FashionGallery {
  constructor() {
    // defer DOM lookups to the gallery root so this class can be constructed
    // before the HTML is injected; lookups may return null and are guarded later.
    const root = document.getElementById(ROOT_ID);
    this.viewport = root ? root.querySelector<HTMLElement>("#viewport") : null;
    this.canvasWrapper = root ? root.querySelector<HTMLElement>("#canvasWrapper") : null;
    this.gridContainer = root ? root.querySelector<HTMLElement>("#gridContainer") : null;
    this.splitScreenContainer = root ? root.querySelector<HTMLElement>("#splitScreenContainer") : null;
    this.imageTitleOverlay = root ? root.querySelector<HTMLElement>("#imageTitleOverlay") : null;
  ...
  enterZoomMode(selectedItemData) {
    ...
    getRoot().classList.add("zoom-mode");
    ...
    if (this.closeButton) this.closeButton.classList.add("active");
    const splitLeft = rootQuery("#splitLeft");
    const splitRight = rootQuery("#splitRight");
    if (splitLeft) splitLeft.addEventListener("click", this.handleSplitAreaClick.bind(this));
    if (splitRight) splitRight.addEventListener("click", this.handleSplitAreaClick.bind(this));
    ...
    Flip.fit(this.zoomState.scalingOverlay, selectedElement, {
      ...
      onComplete: () => {
        ...
        if (this.zoomState.scalingOverlay) {
          try { getRoot().removeChild(this.zoomState.scalingOverlay); } catch (e) {}
          this.zoomState.scalingOverlay = null;
        }
        splitContainer.classList.remove("active");
        try { getRoot().classList.remove("zoom-mode"); } catch (e) {}
  ...
  initDraggable() {
    ...
    this.draggable = Draggable.create(this.canvasWrapper, {
      ...
      onDragStart: () => {
        getRoot().classList.add("dragging");
      ...
      onDragEnd: () => {
        getRoot().classList.remove("dragging");
  ...
  handleMouseLeave() {
    ...
    if (rootEl && rootEl.classList.contains("dragging")) {
    ...
    gsap.to(
      this.gridItems.map((item) => item.element),
      {
        ...
        onComplete: () => {
          ...
          const percentageIndicator = this.controlsContainer ? this.controlsContainer.querySelector(".percentage-indicator") : null;
          const switchElement = this.controlsContainer ? this.controlsContainer.querySelector(".switch") : null;
          const soundToggle = this.controlsContainer ? this.controlsContainer.querySelector(".sound-toggle") : null;
  ...
  updatePercentageIndicator(zoomLevel) {
    ...
    // scope lookup to gallery root first to avoid null when component isn't mounted globally
    const root = document.getElementById(ROOT_ID);
    const el = root ? root.querySelector("#percentageIndicator") : document.getElementById("percentageIndicator");
    if (el) {
      (el as HTMLElement).textContent = `${percentage}%`;
    }
  }
  ...
  setupEventListeners() {
    ...
    // global mouseleave is intentionally disabled earlier; keep viewport-only listener if available
    if (this.viewport) this.viewport.addEventListener("mouseleave", () => this.handleMouseLeave());
...
// small helper to query inside root without throwing
function rootQuery(selector: string): HTMLElement | null {
  const root = document.getElementById(ROOT_ID);
  return root ? (root.querySelector(selector) as HTMLElement | null) : null;
}

// Export classes so app/gallery/page.tsx can import and instantiate them.
// Do NOT auto-initialize here in module scope (avoids SSR/multiple-init issues).
export { PreloaderManager, FashionGallery };

/* Optional auto-init for standalone testing (commented out on purpose)
let gallery;
